...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (user.password !== user.passwordConfirm) return next(new AppError('Passwords are not the same', 500));
  user.passwordResetToken = undefined;
  user.passwordResetExpires = undefined;
  await user.save();
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  // Check if posted currrent passowrd is correct
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(new AppError('Password is wrong! you have 2 more trials', 401));
  }
...
// exports.restrictTo = (admin, leadGuide) => {
//   return (req, res, next) => {
//     // roles ['admin', lead-guide]. role = 'user'
//     if (req.user.role !== admin && req.user.role !== leadGuide) {
//       return next(new AppError('You do have permission to perform this action!', 403));
//     }
//     next();
//   };
